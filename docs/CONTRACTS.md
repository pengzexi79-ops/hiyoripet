...
## C10 衣柜换装与动作扩展（2026-09-07）
- 换装：右键连点三下（340ms 判定窗）打开衣柜面板；六个部位（上衣/内衣/下衣/袜子/鞋子/领结）各含色卡，点击即时重新着色纹理。单/双击右键仍是 API 面板开关；右键长按仍是收纳箱。
- 实现：`src/core/outfit.ts` 对 Hiyori.2048/texture_01.png（衣物图集）按归一化矩形掩码做"目标色 × 最大通道亮度"重着色，保留褶皱明暗；通过 PIXI 全局缓存的 BaseTexture + CanvasResource 热替换纹理。选色持久化在 localStorage(`hiyori-outfits-v1`)，启动时自动恢复（纹理未就绪自动重试 12s）。
- 掩码是 Hiyori 专用实测值；其他模型需要自定义掩码或放入整张替换图集。内衣部位受模型网格限制，仅领口内搭可变色。
- 动作：新增 climb（攀爬，双臂交替上够+蹬腿，桌面大幅向上移动触发）、sit（正坐）；kneel（蹲）/duck-sit（鸭子坐）参数重调。坐姿族在空闲随机行为中轮换。
- 表情：applyFace 新增 angry/sad/proud/sleepy 参数化表情，随机互动池同步扩展。
- 衣物物理：拖拽/甩窗/游走速度经弹簧驱动 ParamSkirt/ParamSkirt2/ParamShoulder/ParamBustY（Hiyori 专用裙摆参数）。
- 面板定位：气泡/收纳箱/API 面板/衣柜统一头部锚定（gap 8px），默认开在宠物眼睛朝向的一侧（petFacing：指针悬停与游走方向驱动），空间不足才换边；扩展列宽度按最宽面板（API 286）预留。
- 命中区域换算升级为 缩放(k) + 画布中心位移：侧栏展开/收起平移模型中心后区域同步平移，修复右键长按展开时本体"画面缺失"。
