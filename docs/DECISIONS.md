...
## D19 右键长按开箱 + 头部锚定气泡 + 拖喂反馈重做（2026-09-06）
- 用户裁定：收纳箱打开手势改为右键长按 650ms（旧左键长按删除）；右键短按仍为 API 面板开关。
- 气泡定位从"包围盒 + 固定扩展列偏移"改为"头部不透明带锚定"：复用 getOpaqueRegions 的 WebGL 回读缓存（400ms TTL），不新增回读开销；气泡永远不与人物不透明像素重叠，水平间隙 10px。
- 拖喂交互参考 Shimeji-ee 的注视/接物反馈范式：拖入悬停时视线跟随 + 张嘴等待，drop 后多文件全部入库并按"接住→咀嚼→播报"反馈；单次拖入多个文件逐条调用 POST /api/box。
- 桌面 `.url` 与 `.lnk` 同等对待（吃掉+可还原）；`.exe`/普通文件坚持 D18 的"只登记不移动"决策不变。

## D20 命中区域保真 + DPI 归一 + 面板贴头（2026-09-07）
- 用户裁定：点击双腿间隙必须穿透到桌面；气泡/设置在头部左右、贴近但不挨住不遮挡；修复缩放断层/手脚消失/连续操作卡住。
- 采用 `--force-device-scale-factor=1` 归一 DPI，而不是适配 dpr：窗口 LogicalSize 本就 1:1 物理，WebView2 独自缩放 1.3725 是全部坐标错乱的根源；归一后所有 *dpr 数学严格成立，渲染 1:1 物理像素。
- 放弃"区域矩形按行合并换性能"的旧折中：ExtCreateRegion 批量建区域可承载数百矩形，行合并会把空隙填平；阈值只留 900 作病态兜底。
- 姿态动画期以"缓存失效 + 多点补同步"替代逐帧提交：回读有 400ms TTL，补同步点覆盖姿态缓动时程。

## D21 衣柜换装（纹理重着色）与动作/朝向体系（2026-09-07）
- 换装路线取舍：Hiyori 无换装参数（cdi3.json 70 参数均为动作/表情），无独立服装部件；社区常用的"部件显隐"方案不可行。选择官方 texture swap 思路 + 运行时 Canvas 重着色：按部位掩码改写衣物图集像素，经 PIXI BaseTexture.setResource(CanvasResource) 热替换，不落盘、不影响原始资源。
- 部位掩码为 Hiyori 实测归一化值（texture_01.png 为纯衣物图集，皮肤/头发在 texture_00，天然隔离）；跨模型复用需重新标定或使用整图替换包（预留 outfits/ 目录约定）。
- 内衣部位诚实降级：模型没有对应网格，仅领口内搭可变色；面板内展示说明，不做假效果。
- 右键手势最终语义：单击/双击=API 面板（340ms 判定），三连击=衣柜，长按 650ms=收纳箱，收纳箱打开时单击=关闭。判定互斥已覆盖。
- 气泡/面板选边以 petFacing（指针悬停、游走方向）为准，空间不足才换边；扩展列按最宽面板预留，消除"面板被钳到远处"的视觉割裂。
