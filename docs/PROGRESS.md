...
## 命中保真/缩放断层/气泡贴头（2026-09-07）
- ✅ additionalBrowserArgs 强制 device-scale-factor=1；命中区域取消横条合并；姿态变化多点补同步区域；气泡/API 面板头部锚定 gap=8px。
- 门禁：tsc / vite build / cargo check / Tauri NSIS（见提交）。

## 命中区域可见性优先重构（2026-09-07 深夜）
- ✅ 真机复现并定位"动作摆动时手脚消失/只剩碎片"根因：本机 WebGL extract 回读会间歇性损坏（只返回腿部条带/空帧，总面积还接近正常值），坏矩形提交 SetWindowRgn 后直接裁掉人物。
- ✅ 重构为 goodRects 可信区域：只有通过完整性校验（覆盖率≥4% 且不透明并集高/宽≥包围盒 50%）的回读才入库；校验失败退化为整个包围盒区域（人物 100% 可见，空白处暂时吃点击），回读恢复后自动回到精细轮廓。
- ✅ 修正并行会话引入的两个问题：中心化区域换算在窗口尺寸变化后会把区域算到负坐标（宠物整体消失），改回 contain 原点等比缩放；syncRendererSize resize 时清空缓存会导致放大后必然走 2.2x 回读，改为保留 goodRects 按倍率换算。
- ✅ 真机验证：2.2 倍缩放 + 连点动作，人物完整可见无碎片（v13-final-pose.png），区域为包围盒单矩形（回读不健康时的预期形态）。
- ⚠️ 本轮验证期间发现另一 AI 会话在同一仓库并行开发并提交（635eaee），已把双方改动合并收敛；请勿再让两个会话同时改这个仓库，否则必然互相覆盖。

## 衣柜换装/衣物物理/攀爬与坐姿（2026-09-07 凌晨）
- ✅ 衣柜：右键连点三下打开，六部位色卡（上衣/内衣/下衣/袜子/鞋子/领结）即时换装；实现为"归一化掩码重着色衣物图集 + 预填充 PIXI 纹理缓存 + 模型热重载"。真机确认樱花粉开衫生效（w8-outfit-reload.png）。选色持久化 localStorage，启动自动恢复。
- ⚠️ 走过的弯路（防复发）：运行时改写 PIXI 纹理像素、替换 ImageResource.source、直接对 GL 句柄 texImage2D 都不会改变画面——pixi-live2d-display 的 Cubism 渲染器只持有自己的 GL 句柄且 PIXI 不触发重上传；setResource 仅可调用一次会抛错。最终采用缓存预填充 + pet.load 热重载（唯一可靠路径）。
- ✅ 衣物物理：弹簧摆动新增 ParamSkirt/ParamSkirt2/ParamShoulder/ParamBustY，拖拽/甩窗/游走速度驱动裙摆与肩部。
- ✅ 动作：新增 climb 攀爬（大幅向上移动触发，双臂交替上够）、sit 正坐；kneel/duck-sit 参数重调；新表情 angry/sad/proud/sleepy 并入随机互动池。
- ✅ 面板定位：气泡/收纳箱/设置/衣柜统一头部锚定 + 眼睛朝向优先选边（petFacing），扩展列起点不再做下限钳制，gap=8px 贴身不遮挡。
- ✅ 修复右键长按展开时本体缺失：区域换算补上"画布中心位移"维度（缩放 k + 中心平移），展开/收起时区域随模型正确平移。
- ✅ 门禁：tsc / vite / Tauri NSIS 全绿。

## 交互分级 / 衣柜穿脱 / 步态重做（2026-09-07 夜）
- ✅ 右键交互改为分级计时判定：每击重置 1s 定时器，到点按连击数分发（1=API 设置，2=收纳箱，3=衣柜）。CDP 实测三分支均正确且互斥。
- ✅ 修"最大缩放后整窗无法点桌面"：彻底移除缩放期间的整窗区域提交（该区域一旦未恢复就整窗遮挡桌面），改为 scaleGood 精确换算 + 可见性优先 bbox 兜底。真机验证：最大缩放 792×1320 时区域=单矩形宠物包围盒 (15,126)-(776,1193)，宠物周围桌面可正常点击。
- ✅ 衣柜 v2：穿上/脱下真实生效（脱下=掩码区域擦成透明，露出水手服本体，真机确认）；掩码从"截图目测"改为 Python PIL 连通域实测标定（修复旧掩码把大腿皮肤标进袜子导致的破洞、开衫染色越界）；重着色跳过近白像素保住水手服条纹；部位扩充为 外套/上衣(水手服领)/内衣/下衣/袜子/鞋子/装饰。
- ✅ 步态重做：迈腿幅度 0.34→0.55、双臂摆动 0.26→0.36、身体起伏加倍；新增 run 跑步姿态（远距离游走触发）；肢体（手臂）加入拖拽弹簧物理。
- ✅ 进食演出补咀嚼动画（ParamMouthOpenY 时间窗强制起伏）；空闲自聊频率上调（60-130s 周期）。
- ✅ 桌面感知闲聊：后端 55-80s 轮询前台窗口标题（ctypes，仅标题不读键入），变化时广播 desktop-activity，前端按 API 配置走 LLM 场景评论或本地模板。
- ⚠️ 教训：批量文本替换脚本曾误删 maxZoomLevel/onNativeRegionResize 导致缩放抛异常，已从 git 历史救回；大规模删除务必逐函数校验。
- 门禁：tsc / vite / compileall / cargo check / Tauri NSIS 全绿。
